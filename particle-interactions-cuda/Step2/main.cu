...
int main(int argc, char **argv)
{
  ...
  */
  ...
  // it is assumed that blockDim is divisible by 32
  // -- meaning no padding is needed and no out of bounds access is possible
  const std::size_t sharedMemSize = blockDim.x * sizeof(float) * 7;
  // const std::size_t sharedMemSize = blockDim.x * sizeof(float4) * 2;
  ...
  for (unsigned s = 0u; s < steps; ++s)
  {
    ...
    calculateVelocity<<<gridDim, blockDim, sharedMemSize>>>(dParticles[srcIdx], dParticles[dstIdx], N, dt);
  }
