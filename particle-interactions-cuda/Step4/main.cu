...
int main(int argc, char **argv)
{
  ...
  */
  ...
  // we will use 3 streams, as stated in the assignment doc:
  // - one for velocity calculation,
  // - one for center of mass calculation
  // - one for data transfer during iterations that satisfy shouldWrite() condition

  cudaStream_t velocityStream;    CUDA_CALL(cudaStreamCreate(&velocityStream));
  cudaStream_t comStream;         CUDA_CALL(cudaStreamCreate(&comStream));
  cudaStream_t transferStream;    CUDA_CALL(cudaStreamCreate(&transferStream));

  cudaEvent_t velocityDone;       CUDA_CALL(cudaEventCreate(&velocityDone));
  cudaEvent_t comDone;            CUDA_CALL(cudaEventCreate(&comDone));
  cudaEvent_t transferDone;       CUDA_CALL(cudaEventCreate(&transferDone));
  ...
  for (unsigned s = 0u; s < steps; ++s)
  {
    ...
    /******************************************************************************************************************/
    calculateVelocity
      <<<simGridDim, simBlockDim, sharedMemSize, velocityStream>>>
      (dParticles[srcIdx], dParticles[dstIdx], N, dt);
    CUDA_CALL(cudaEventRecord(velocityDone, velocityStream));

    if (shouldWrite(s)) {
      const auto recordNum = getRecordNum(s);

      CUDA_CALL(cudaEventSynchronize(velocityDone));
      centerOfMass
        <<<redGridDim, redBlockDim, redSharedMemSize, comStream>>>
        (dParticles[srcIdx], dCenterOfMass, dLock, N);
      CUDA_CALL(cudaEventRecord(comDone, comStream));
      CUDA_CALL(cudaMemcpyAsync(hParticles.posx, dParticles[srcIdx].posx, sizeof(float) * N, cudaMemcpyDeviceToHost, transferStream));
      CUDA_CALL(cudaMemcpyAsync(hParticles.posy, dParticles[srcIdx].posy, sizeof(float) * N, cudaMemcpyDeviceToHost, transferStream));
      CUDA_CALL(cudaMemcpyAsync(hParticles.posz, dParticles[srcIdx].posz, sizeof(float) * N, cudaMemcpyDeviceToHost, transferStream));
      CUDA_CALL(cudaMemcpyAsync(hParticles.velx, dParticles[srcIdx].velx, sizeof(float) * N, cudaMemcpyDeviceToHost, transferStream));
      CUDA_CALL(cudaMemcpyAsync(hParticles.vely, dParticles[srcIdx].vely, sizeof(float) * N, cudaMemcpyDeviceToHost, transferStream));
      CUDA_CALL(cudaMemcpyAsync(hParticles.velz, dParticles[srcIdx].velz, sizeof(float) * N, cudaMemcpyDeviceToHost, transferStream));
  ...
  CUDA_CALL(cudaEventSynchronize(velocityDone));
  CUDA_CALL(cudaEventSynchronize(transferDone));
  centerOfMass<<<redGridDim, redBlockDim, redSharedMemSize, comStream>>>
    (dParticles[resIdx], dCenterOfMass, dLock, N);

  CUDA_CALL(cudaStreamSynchronize(comStream));
  ...
  CUDA_CALL(cudaEventDestroy(velocityDone));
